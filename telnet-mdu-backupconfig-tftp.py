...
def send_backup_tftp_cmd(ip, username, password, hostname, tftpserver):
...
if __name__ == '__main__':
    ...
    for each_line in selected_switch_file.readlines():
        ...
        if check_ipv4_validity(ipv4) and host_is_up(ipv4):
            _output = send_backup_tftp_cmd(ipv4, username, password, hostname, tftpserver)
            # if "Backing up files is successful" in + _output:
            if _output.find("Backing up files is successful") != -1:
